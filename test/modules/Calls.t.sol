...
contract CallsTest is AdvTest {
  ...
  function test_createContract_success(
    ...
    // fund CallsImp so it can forward ETH
    vm.deal(address(calls), _value);

    // We only care that the event is emitted – we do not validate the data
    vm.expectEmit(false, false, false, false, address(calls));
    emit Calls.CreatedContract(address(0));

    vm.prank(address(calls));
    address newAddr = calls.createContract{ value: _value }(code);
  ...
  function test_create2Contract_success(uint256 _value, bytes32 _salt) external {
    ...
    vm.prank(address(calls));
    address deployed = calls.create2Contract{ value: _value }(code, _salt);

    bytes32 data = keccak256(abi.encodePacked(bytes1(0xff), address(calls), _salt, keccak256(code)));
  ...
  function test_create2Contract_duplicate(
    ...
    vm.prank(address(calls));
    calls.create2Contract(code, _salt); // first deployment succeeds

    // second deployment with same salt & code must revert
    vm.expectRevert(abi.encodeWithSelector(Calls.Create2Failed.selector, code, _salt));

    vm.prank(address(calls));
    calls.create2Contract(code, _salt);
  }
